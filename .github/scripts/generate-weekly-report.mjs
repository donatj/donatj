...
const response = await new OpenAI().responses.create({
	...
	input: `Research what ${username} has been up to in the past 14 days. Prioritize public GitHub activity—especially pull requests, commits, issues, releases, and repositories—across ${username}'s personal account and every public GitHub organization associated with ${username}. Do not limit research to personal repositories. Use other public sources when useful. Include only things that actually happened; do not mention missing, absent, unverified, or undiscovered activity. Ignore Dependabot-related work. Every pull request you mention must be an exact direct Markdown link to its public GitHub URL; never invent a link. Use link text that describes what the pull request achieves, not a repository name or pull-request number. Never use or mention non-programming-related Reddit activity or anything political. Do not include statements or opinions by ${username} that could be critical of any person, organization, project, or product.
